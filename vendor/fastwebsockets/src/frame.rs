...
impl Payload<'_> {
  /// Extract the inner BytesMut if this is a Bytes variant (zero-copy).
  /// Falls back to copying for other variants.
  #[inline(always)]
  pub fn into_bytes_mut(self) -> BytesMut {
    match self {
      Payload::Bytes(b) => b,
      Payload::Owned(v) => BytesMut::from(v.as_slice()),
      Payload::Borrowed(b) => BytesMut::from(b),
      Payload::BorrowedMut(b) => BytesMut::from(&*b),
    }
  }
